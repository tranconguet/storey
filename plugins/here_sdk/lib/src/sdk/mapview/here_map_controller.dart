...
typedef TakeScreenshotCallback = void Function(Image image);
...
class HereMapController implements MapViewBase {
  ...
  /// Asynchronously retrieves screenshot of map view
  ///
  /// @param callback Completion handler called when the screenshot is completed
  void takeScreenshot(TakeScreenshotCallback callback) {
    if (callback == null) {
      return;
    }
    _mapViewInternal?.internaltakeScreenshot((Uint8List bytes) {
      Image image = null;
      try {
        image = bytes != null ? Image.memory(bytes) : null;
      } catch (e) {
