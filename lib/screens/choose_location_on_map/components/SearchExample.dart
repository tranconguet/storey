...
class SearchExample {
  ...
  SearchExample(
    ...
    _camera.lookAtPointWithDistance(
        GeoCoordinates(21.0294498, 105.8544441), distanceToEarthInMeters);
  ...
  void moveToGeoCoordinate(GeoCoordinates geoCoordinates) {
    const double distanceToEarthInMeters = 600;
    _hereMapController.pinWidget(_createWidget(), geoCoordinates);
  ...
  void _searchExample(String text) {
    print("Searching in viewport for: " + text);
    _searchInViewport(text);
  }
  ...
  Future<void> autoSuggest(String text) {
  ...
  Future<void> _handleSuggestionResults(
    ...
    for (Suggestion autosuggestResult in list) {
      _suggestionList.add(autosuggestResult);
